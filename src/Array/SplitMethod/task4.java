...
public class task4 {
    public static void main(String[] args) {
        String str = "Hummingbirds are birds native to the Americas and comprise the biological family " +
                ...
                " 51 mph)";
        // find the words with even number of letters
        String text = "";
        for (int i = 0; i < str.length(); i++) {
            if (Character.isAlphabetic(str.charAt(i)) || str.charAt(i) == ' ') {
                text += str.charAt(i);// this loop creates a new string with words only, removing any other characters
            }
        }
        System.out.println(text);
        System.out.println("============");
        String[] words = text.split(" "); //split();  separate given string from desired char/index
